...
const ESTADOS_CONTRATO = [
  ...
  { value: 'Fiscalia', label: 'En Fiscalía' },
  { value: 'Liquidado', label: 'Liquidado' },
  { value: 'ParaDenuncio', label: 'Para Denuncio' },
  { value: 'Robada', label: 'Robada' },
  { value: 'Termino', label: 'Terminado' },
  { value: 'Activo', label: 'Activo' }
];

export function ContratosExportModal({ isOpen, onClose, contratos }: ContratosExportModalProps) {
  const [tipoExportacion, setTipoExportacion] = useState<ExportOptionType>('completo');
  const [fecha, setFecha] = useState('');
  const [fechaInicio, setFechaInicio] = useState('');
  const [fechaFin, setFechaFin] = useState('');
  const [estado, setEstado] = useState(ESTADOS_CONTRATO[0].value);
  ...
  const handleExport = () => {
    ...
    exportarContratosExcel(contratos, opciones);
    onClose();
  };
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-colors hover:bg-slate-50 [&:has(:checked)]:border-primary [&:has(:checked)]:bg-primary/5">
              <input
                ...
                name="exportType"
                value="estado"
                checked={tipoExportacion === 'estado'}
                onChange={(e) => setTipoExportacion(e.target.value as ExportOptionType)}
                className="w-4 h-4 text-primary border-slate-300 focus:ring-primary"
              />
              <div className="flex flex-col flex-1">
                <span className="text-sm font-medium text-slate-700 mb-2">Por estado</span>
                {tipoExportacion === 'estado' && (
                  <select
                    value={estado}
