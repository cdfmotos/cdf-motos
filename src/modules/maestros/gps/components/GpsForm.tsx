...
interface GpsFormProps {
  gps: GPS | null;
  onClose: () => void;
  onSave: (data: Partial<Omit<GPS, 'id' | '_sync_status' | 'created_at'>>) => Promise<void>;
  loading: boolean;
}
      ...
      <div className="bg-card w-full max-w-lg rounded-xl shadow-2xl flex flex-col">
        ...
        <div className="p-6">
          <form id="gps-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">IMEI del GPS *</label>
              <input 
                type="text" 
                name="gps_imei" 
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Simcard</label>
              <input 
                type="text" 
                name="simcard" 
                value={formData.simcard} 
                onChange={handleChange}
                placeholder="Número de línea o ICCID"
                className="w-full px-3 py-2 border border-border rounded-lg focus:ring-1 focus:ring-primary focus:border-primary outline-none" 
              />
            </div>
