...
import { exportarGastosExcel } from '../utils/exportExcel';
import type { ExportOptionType, ExportOptions } from '../utils/exportExcel';
import type { Gasto } from '../../../db/schema';

interface GastosExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  gastos: Gasto[];
}

export function GastosExportModal({ isOpen, onClose, gastos }: GastosExportModalProps) {
  ...
  const handleExport = () => {
    ...
    exportarGastosExcel(gastos, opciones);
    onClose();
  };
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-2 text-slate-800">
            ...
            <h3 className="font-medium">Exportar Gastos a Excel</h3>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition-colors hover:bg-slate-50 [&:has(:checked)]:border-primary [&:has(:checked)]:bg-primary/5">
              ...
              <div className="flex flex-col flex-1">
                <span className="text-sm font-medium text-slate-700 mb-2">Por día de gasto</span>
